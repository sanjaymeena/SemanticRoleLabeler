...
public class FeatureGenerator implements Serializable {
	...
	public Feature getFeature(FeatureName fn, boolean includeAllWords, String POSPrefix, BrownCluster bc) {
		...
		if (cache.containsKey(fn)) {
			...
			case PredParentWord:
				ret = new PredDependentAttrFeature(fn, WordData.Form, TargetWord.PredParent, includeAllWords,
						POSPrefix);
				break;
			case PredParentPOS:
			...
			case POSPath:
				ret = new PathFeature(fn, WordData.POS, POSPrefix);
				break;
			case DeprelPath:
				ret = new PathFeature(fn, WordData.Deprel, POSPrefix);
				break;
			case Position:
			...
			default:
				if (fn.toString().startsWith("Brown")) {
					...
					TargetWord tw = TargetWord
							.valueOf(fn.toString().substring("Brown".length() + cv.toString().length()));
